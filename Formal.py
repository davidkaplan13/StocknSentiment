...
class Twitter(object):
    def __init__(self):
        ...
        self.api = API(
            self.auth)  # Used for the search method in the Main Function
    ...
    def Trends(self):
        ...
        Trend_Data = Trend_Data[
            'trends']  # Creates a data frame for Trend data
    ...
    def Remove_URL(self, tweet):
        ...
        """
        try:
            noUrl = re.sub(r'http\S+', '', str(
                tweet.
                text))  # Uses regex module to convert URL patterns to spaces
    ...
    def IdentifyEmoji(self, CleanTweet):
        ...
        """
        try:
            noEmoji = emoji.demojize(
                CleanTweet
            )  # Uses Emoji module to convert emoji image to description
    ...
    def ClassifyWords(self, FullyCleanTweet):
        ...
        PosFile = open(
            "PositiveWords.txt").read()  # Reads the positive word files
        NegFile = open(
    ...
    def Main(self):
        ...
        try:
            ...
            if OverallSentiment > 0:
                ...
                print(
                    "Hence, Negative"
                )  # If the overall sentiment is smaller than 0, then overall sentiment of 20 parsed tweets is negative
            else:
                print(
                    "Hence, Neutral"
                )  # Prints Neutral if overall sentiment not classified in the IF statments above.

        except error.TweepError as e:
            print(
                e.reason)  # Prints Error of Twitter Search method to the user
            print("error")  # Basic Error Handling
...
class Stock(object):
    ...
    def StockData(self):
        ...
        try:
            print(
                stockentry
            )  # Print Statement showing user entry; done for compeltion purposes
            global Data
            data = quandl.get(
                'EOD/' + str(stockentry), rows=7
            )  # Retrieves the last 7 days end of day prices for the user selected company
            print(data)
            Data = data
...
class Window(Frame):
    ...
    def main_window(self):
        ...
        self.Twitter.Trends(
        )  # Calls on the twitter trend function by referral statment

        self.canvas = Canvas(
            self.master, width=670, height=450)  # Initiate the canvas size
        self.canvas.grid(
            row=0, column=0,
            sticky='nsew')  # Places the canvas at row 0 and column 0
        ...
        self.var.set(
            self.Choice[0])  # Displays the first ticker as the pre-set
        self.w = OptionMenu(self.master, self.var, *self.Choice)
    ...
    def CompanyEntry(self):
        ...
        try:
            ...
            self.ButtonVG.place(
                x=350, y=350)  # Places the button on the screen
            ...
            )  # Calls on the class stock, to pull 'EOD' data using the query
    ...
    def TwitterQueryEntry(self):
        try:
            ...
            print(query)
            self.Twitter.Main(
            )  # Calls on the function main in the twitter class
...
class HelpPage(Frame):
    ...
    def create_Help_Page(self):
        self.masters.title(
            "Help Page")  # Sets the title of the page to help page
        self.masters.configure(
            background='white', highlightbackground='light steel blue')
        # Sets the background of the help page to white
        self.canvashp = Canvas(
            self.masters, width=670, height=480)  # Creates a canvas
        self.canvashp.grid(
            row=0, column=0,
            sticky='nsew')  # Places the canvas at row 0 column 0
        ...
        self.w = OptionMenu(self.masters, self.var, *self.Choice)
    ...
    def getEntry(self):
        ...
        if entry == "Help With Stock Graph":
            ...
            self.HelpTwitter(
            )  # Redirects and calls on functiono help twittter
        else:
            self.About()

    def HelpStock(self):

        choice = ["AAPL", "MCD", "MSFT", "NKE", "INTC", "BA",
                  "DIS"]  # choices of tickers
        Ticker_Names = [
        ...
        for i in range(
                0,
                len(choice)):  # Iterates from 0 to the length of choice array
        ...
        PrettyTicker_Name = json.dumps(
            Ticker_Name, indent=2
        )  # Idents the dictionary using the module json, for a nicer display
        self.LabelHS = Label(
            self.masters,
...
class StockPage(Frame):
    ...
    def GraphStock(self):
        self.mastersp.title(
            "Tweet and Graph Page")  # Sets the title of the page
        print(OverallTotalToPlot)
        df = pd.DataFrame({
            'values': OverallTotalToPlot
        })  # Appends values of sentiment to a pandas dataframe
        self.canvashp = Canvas(
            self.mastersp, width=670, height=480)  # Creates a canvas
        self.canvashp.grid(
            row=0, column=0,
            sticky='nsew')  # Displays canvas at row = 0 and column =0
        self.rectangle_top = self.canvashp.create_rectangle(
            0, 0, 660, 45, fill='light sky blue', outline='light sky blue')
        ...
        self.Choices = ["Bollinger Bands", "Moving Average",
                        "Candlestick"]  # Creates Choices
        self.varc.set(
            self.Choices[0])  # Sets the first chocie as a pre-set for display
        ...
        self.canvashp.create_rectangle(
            10, 200, 650, 400, outline='black',
            fill='white')  # creates a rectangle
        self.canvashp.update()  # Updates the canvas

        positive_tweets_lines = json.dumps(
            positive_tweets, indent=2
        )  # Indents the positive tweets with lines for a better display
        self.LabelPT = Label(
            self.mastersp,
        ...
        )  # Indents the negative tweets with lines for a better display
        self.labelNT = Label(
            self.mastersp,
            text=negative_tweets_lines,
            font=("Avenir", 6),
            foreground='tomato')  # Creates the text label
        ...
        fig.suptitle(
            stockentry + ' STOCK DATA',
            fontsize=12)  # Creates a title to be plotted on the figure

        ax = plt.subplot2grid((2, 2), (0, 0), colspan=4,
                              rowspan=1)  # Adds a subplot to the figure
        Data['Date'] = Data.index.map(
            mdates.date2num
        )  # Converts dates to number format, to allow for it to be plotted
        plt.plot(
            Data['Close'], Label="Close", color="yellow"
        )  # Plots closing price; Adds a label on the line, and sets the color of the line to yellow
        plt.plot(
            Data['MA50'], label="Moving Average", color="blue"
        )  # Plots the moving average; Adds a label for the line, and sets the color of the line to blue
        plt.plot(
            Data['UpBB'], Label="Upper Bollinger Band", color="black"
        )  # Plots the upper bollinger band; sets the label and sets the color of the line
        plt.plot(
            Data['LowBB'], Label="Lower Bollinger Band", color="grey"
        )  # Plots the lower bollinger band; sets the label and sets the color of the line
        plt.xlabel("Date")  # Places x-axis label of Date
        ax.xaxis.set_major_formatter(mdates.DateFormatter(
            '%m-%d'))  # Formats he axis to months and days
        plt.legend(
            loc='best')  # Places the key in the best location on the graph
        ax.set_ylabel('Value')  # Assigns the y label to Value

        Data['Date'] = Data.index.map(
            mdates.date2num
        )  # Converts the dates to number format to allow for it to be plotted
        candlestickData = Data[[
            'Date', 'Open', 'High', 'Low', 'Close'
        ]]  # Creates and array full of the data needed to generate a candlestick graph
        candlestick_ohlc(
            ax,  # Plots the candlestick graph on the main ax
            candlestickData.
            values,  # uses the candlestick data and extract the values of each column
        ...
        ax4.xaxis.set_major_formatter(
            mdates.DateFormatter('%d'))  # formats the x-axis dates as days
        ...
        ax5.scatter(
            df, df['values'], label='Sentiment Value',
            color='blue')  # scatters the tweet sentiment values
        cofficient = pearsonr(
            (df['values'][0:7]),
            Data['Close'])  # Creates the correlation coeffienct
        print("correlaction coefficient: ", str(cofficient))
        ax5.plot(
            cofficient,
            label=
            'Pearson correlation coefficient',  # Plots the correlation Coefficient
            color='magenta')
        ax5.set_xlabel('Tweet')  # Adds an x-axis label
        ax5.legend(
            loc='best')  # Adds the key to the best location on the subplot
